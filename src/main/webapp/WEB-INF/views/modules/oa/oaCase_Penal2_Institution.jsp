...
<body>
	...
	<form:form id="inputForm" modelAttribute="oaCase" action="${ctx}/oa/oaCase/saveCase" method="post" class="form-horizontal">
		...
		<fieldset>
			<legend>承办机构行政处罚审批：${oaCase.title}</legend>
			<table class="table-form">
				<c:if test="${oaCase.rejectFlag==true}">
					<tr><td class ="redtit" colspan="6"><h4>该流程被驳回</h4></td></tr>
					<tr><td class ="tit">原因</td><td colspan="5">${oaCase.caseMgtCenterPenalOption}</td></tr>
				</c:if>			
				<!-- 案件简报 -->
				...
				<!-- 时间进展 -->				
				<tr><td class="tit" colspan="6"><h4>承办机构审批</h4></td></tr>	
				<tr>
					<td class="tit" rowspan="2">承办人意见：</td>
					<td colspan="4">${oaCase.normAssigneePenalOpt}</td>
				</tr>
				...
				<tr>
					<td>承办机构意见</td>
					<td colspan="5">
						<form:textarea path="institutionPenalOption" class="required" rows="3" maxlength="300"/>
					</td>	
				</tr>				
			</table>
		</fieldset>
